...
- Keep the headline, subhead, and lede aligned on one verified development; a clean, source-tight package is often enough to publish.
...
- [columnist] When the digest is sparse but the quote is blunt, make the quote itself the lede and attach only the confirmed follow-up fact.
...
- [columnist] Don’t write broad “relationship” or “usually” logic around a sparse digest; when there are no extra source facts, avoid generic speculation and keep the angle simple and specific.
- [columnist] Don’t wrap a blunt quote in report-framing or scene-setting when the quote and one confirmed follow-up fact are already enough.
- [columnist] Don’t build an article from facts that are not actually present in the source digest; if rewrite still relies on unsupported claims, the piece should be dismissed.
