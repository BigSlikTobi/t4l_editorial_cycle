...
- Add useful team context only when it stays inside the source-supported frame and does not introduce new claims.
- A clean quoted line can be the story’s hook when it concisely captures the verified point.
...
- [insider] When the source is only a live, unresolved development, keep the headline and lede tight and let the confirmed status update do the work.
...
- Don’t turn a clean reporting update into holdout speculation when the source only supports a report-to-camp declaration.
