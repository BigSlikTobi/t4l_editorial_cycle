...
- Include the key details the source actually gives instead of flattening them into vague summary.
- A full list or specific supporting detail can strengthen the piece when it stays inside the verified package.
- When the update is personal and sensitive, a clear confirmation quote can add value if it confirms facts rather than adding color.
...
- Do not use extra framing that goes beyond the digest; keep only who is involved, what is being investigated, and the timing.
...
- [source-side, not fixable in prose] Do not import specific names or details when the digest is empty and the package cannot be grounded against it.
- [source-side, not fixable in prose] Do not try to force a coherent rewrite when the digest has no usable facts; unresolved grounding failures are fatal on rewrite attempts.
- [source-side, not fixable in prose] Do not repeat unsupported prompt details when the digest itself is effectively empty; there is no factual base to salvage.
