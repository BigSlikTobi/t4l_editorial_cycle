...
- Don’t add unsupported color, context, or narrative momentum when the source already provides enough to report cleanly.
- Don’t drift into season outlook or other speculative framing unless the source explicitly supports it.
- Don’t bury the main supported hook under extra framing or explanatory padding.
- Don’t let a headline promise more significance than the body can actually prove.
...
- Don’t turn a brief quote into “what it means” unless the source actually supports that interpretation.
- Don’t repeat the same claim across headline, subhead, intro, and first sentence; each element needs a distinct job.
- Don’t use a sourced quote that isn’t actually present in the digest, and don’t build the piece around it.
- Don’t force a story from an empty digest; if there’s nothing to ground beyond the headline, it’s a source-side, not fixable in prose, failure.
- Don’t proceed when the source digest is effectively empty or missing the key facts needed to support the angle; treat that as a source-side, not fixable in prose, failure.
...
- [casey whitaker] Don’t add unsupported team/feeling framing when the digest only proves a headline-level retirement contemplation; source gaps like that are source-side, not fixable in prose.
- [casey whitaker] Don’t add unsupported internal-temperature, depth-chart, or what-it-means framing when the digest only supports direct praise; keep the note short and quote-bound.
- [casey whitaker] Don’t add quarterback-room speculation or “every snap” implications when the digest only supports the final-season declaration and the Pittsburgh tie-in; keep the consequences out unless they’re explicitly sourced.
- [casey whitaker] Don’t lean on unsupported 2026/Steelers specifics when the source digest does not actually contain those facts; that’s a grounding failure, not an execution tweak.
