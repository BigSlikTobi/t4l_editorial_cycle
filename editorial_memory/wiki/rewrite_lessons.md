...
- For [analyst], tightly grounded status updates work when they pair the key expected action with concrete numbers, trade-offs, or fix-forward context.
...
- Do not broaden a narrow update into a larger narrative about implications, atmosphere, or next steps unless the source supports it.
...
- For [insider], if the source digest is effectively missing or mismatched to the actual article, stop; wrong-team or wrong-player inputs cannot be fixed safely in prose (source-side, not fixable in prose).
