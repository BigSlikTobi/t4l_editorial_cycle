...
- Add only context that clearly comes from the source and helps explain the update.
- A clean wire-style execution works when it stays grounded and removes unsupported flourish.
- [insider] A tightly labeled status-change brief can work when the headline, intro, and body all stay on the same specific subject.
- [insider] Strong grounding and specificity can carry a legal update even without extra color if every sentence stays inside the digest.

## What to avoid
- Don’t overreach beyond source support by adding interpretive language or implied implications.
- Don’t widen a tight update into play-speed, strategy, or broader analysis unless the digest explicitly supports it.
- Don’t add unsupported report detail; if the digest is only a headline-level note, keep the article just as thin. [analyst]
...
- Don’t let the package framing or team field drift from the actual source subject; keep labels aligned with the verified note. [analyst]
...
- Don’t introduce cross-story or unrelated-digest material into a paragraph just to add color.
- Don’t let a clean ranking/contrast story get muddied by body details that belong to a different digest.
- Don’t make the lede/headline relationship awkward by foregrounding a named player or detail before the story’s actual ranking angle is clear.
...
- Don’t introduce unsupported specifics or repeat source-narration/meta framing when the source base cannot support a rewrite (source-side, not fixable in prose).
