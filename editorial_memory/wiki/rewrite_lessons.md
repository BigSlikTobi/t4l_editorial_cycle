...
- Preserve a clear line from headline to lede so the reader immediately gets the point.
- A concise team-specific angle can still work well even when the digest is sparse.
- [analyst] Open competition or future-role uncertainty can work if it stays inside the verified team context.
- [analyst] Keep evaluation or projection language only when the digest explicitly supports the roles and sequence being discussed.
...
- Don’t force a broader frame when the real story is a single decision, promotion, or future-question.
