...
- Keep headline and intro aligned on the same core claim, especially for short updates, but let each do a distinct job.
- Stay tightly inside the verified quote, video, or direct source detail when the digest is sparse.
- Use timing details only when they are explicitly confirmed and genuinely add value.
