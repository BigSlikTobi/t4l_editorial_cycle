...
- Keep the lede tightly anchored to what the source explicitly confirms, especially for personnel, practice, rehab, ceremony, discipline, or cap updates.
- Align headline and lede on the same verified angle without simply repeating identical wording.
- Keep the story tied to the specific game, role, practice setting, event, rehab step, or cap structure the source addresses.
...
## What to avoid
- Do not add unsupported medical, roster, camp, background, quote-attribution, timing, team-specific, or named-player context that the digest does not confirm.
...
- Do not broaden a narrow update into a larger narrative about implications, atmosphere, workload, or next steps unless the source supports it.
...
- For [analyst], avoid status labels like “top pick” or “rookie” unless the source explicitly confirms them; keep the writeup on the supported plan or workload only.
- For [analyst], avoid source-narration phrases that explain what the report does not say; write the verified fact directly instead.
- For [analyst], don’t add game-context claims such as “first live reps of the summer” unless the source actually supplies that framing.
- For [analyst], don’t turn a bare intent statement into a bigger workload or game-plan story when the digest only supports the simple plan to give a player experience.
